...
public class Healthbar : MonoBehaviour
{
    ...
    [Space] [Header("Healthbar Colors:")] public Color highHealthColor = new Color(0.35f, 1f, 0.35f);
    ...
    private void Start()
    {
        ...
        if (healthbarDisplay == null) healthbarDisplay = GetComponent<Slider>();
    ...
    private void Update()
    {
        ...
        if (health < minimumHealth) health = minimumHealth;
    ...
    public void UpdateHealth()
    {
        ...
        if (healthPercentage <= lowHealth && health >= minimumHealth &&
            transform.Find("Bar").GetComponent<Image>().color != lowHealthColor)
        {
            ChangeHealthbarColor(lowHealthColor);
        }
        else if (healthPercentage <= highHealth && health > lowHealth)
        {
            var lerpedColorValue = (float.Parse(healthPercentage.ToString()) - 25) / 41;
        ...
        {
            var lerpedColorValue = (float.Parse(healthPercentage.ToString()) - 67) / 33;
